...
func TestCodeSpanParser(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			want:         *ast.NewAstNode(ast.CODESPAN, []*ast.ASTNODE{ast.NewTextNode([]rune("code span"))}),
			want2:        true,
		},
		{
			name:         "Only Space",
			...
			text:         []rune("`` foo ` bar ``"),
			currentIndex: get_index(0),
			want:         *ast.NewAstNode(ast.CODESPAN, []*ast.ASTNODE{ast.NewTextNode([]rune("foo ` bar"))}),
			want2:        true,
		},
		{
			...
			want:         *ast.NewAstNode(ast.CODESPAN, []*ast.ASTNODE{ast.NewTextNode([]rune("``"))}),
			want2:        true,
		},
		{
			...
			text:         []rune("` a`"),
			currentIndex: new(int),
			want:         *ast.NewAstNode(ast.CODESPAN, []*ast.ASTNODE{ast.NewTextNode([]rune(" a"))}),
			want2:        true,
		},
		{
			...
			text:         []rune("``foo\nbar  \nbaz``"),
			currentIndex: new(int),
			want:         *ast.NewAstNode(ast.CODESPAN, []*ast.ASTNODE{ast.NewTextNode([]rune("foo bar   baz"))}),
			want2:        true,
		},
		{
			...
			text:         []rune("`foo\\`"),
			currentIndex: new(int),
			want:         *ast.NewAstNode(ast.CODESPAN, []*ast.ASTNODE{ast.NewTextNode([]rune("foo\\"))}),
			want2:        true,
		},
	}
